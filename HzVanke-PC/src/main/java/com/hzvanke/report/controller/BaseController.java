...
public class BaseController {
    ...
    public void setOtherThing(String OtherThing) {
        session.setAttribute(AppUtil.SESSION_SYSUSER + "OtherThing", OtherThing);
    }

    public String getOtherThing() {
        Object value = session.getAttribute(AppUtil.SESSION_SYSUSER + "OtherThing");
        if (value != null) {
            return (String) value;
        }
        return "";
    ...
    public SysUser getCurrentUser() {
        if (session.getAttribute(AppUtil.SESSION_SYSUSER) != null) {
            currentUser = (SysUser) session.getAttribute(AppUtil.SESSION_SYSUSER);
        }
        return currentUser;
    }
    ...
    public void setCurrentUser(SysUser currentUser) {
        session.setAttribute(AppUtil.SESSION_SYSUSER, currentUser);
     ...
     * @return
     */
    public List<SysTwolevelmenu> getCurrentUserTwoLevelMenu() {
        if (session.getAttribute(AppUtil.SESSION_TWO_MENU) != null) {
            currentUserTwoLevelMenu = (List<SysTwolevelmenu>) session.getAttribute(AppUtil.SESSION_TWO_MENU);
        }
        return currentUserTwoLevelMenu;
    }

    /**
     * 设置当前用户所有的二级菜单信息
     *
     * @param currentUserTwoLevelMenu
     */
    public void setCurrentUserTwoLevelMenu(List<SysTwolevelmenu> currentUserTwoLevelMenu) {
        session.setAttribute(AppUtil.SESSION_TWO_MENU, currentUserTwoLevelMenu);
        this.currentUserTwoLevelMenu = currentUserTwoLevelMenu;
    }
    //endregion
    ...
    public SysRole getCurrentUserRole() {
        if (session.getAttribute(AppUtil.SESSION_ROLE) != null) {
            currentUserRole = (SysRole) session.getAttribute(AppUtil.SESSION_ROLE);
    ...
    public void setCurrentUserRole(SysRole currentUserRole) {
        session.setAttribute(AppUtil.SESSION_ROLE, currentUserRole);
